...
const udruzenjaService = {
  ...
  getUdruzenjeById: async (id) => {
    const response = await axiosInstance.get(`${API_ENDPOINTS.ORGANIZATIONS}/${id}`);
    return response.data;
  },

  getUdruzenjePredavanja: async (id) => {
    const response = await axiosInstance.get(`${API_ENDPOINTS.LECTURES}?organizationId=${id}`);
    return response.data;
  },

  searchUdruzenja: async (query) => {
    const response = await axiosInstance.get(`${API_ENDPOINTS.ORGANIZATIONS}/search?q=${encodeURIComponent(query)}`);
    return response.data;
  },
