...
const LectureForm = ({ visible, onDismiss, onSuccess, lecture = null }) => {
  ...
  useEffect(() => {
    if (visible) {
      console.log('🔄 LectureForm opened, fetching data...');
      fetchData();
      if (lecture) {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log('🔗 Current API URL:', apiService.getBaseURL ? apiService.getBaseURL() : 'Unknown');
      ...
      // Additional debug for daije structure
      if (Array.isArray(daijeData) && daijeData.length > 0) {
        console.log('🔍 First daija structure:', daijeData[0]);
        console.log('🔍 Daija fields:', Object.keys(daijeData[0]));
      }
  ...
  const handleDaijaSelect = (daija) => {
    console.log('🔄 handleDaijaSelect called with:', daija);
    if (daija === 'custom') {
      console.log('✅ Setting custom speaker mode');
      setUseCustomSpeaker(true);
      handleInputChange('daijaId', '');
      handleInputChange('speaker', '');
    } else {
      console.log('✅ Setting selected daija:', daija);
      setUseCustomSpeaker(false);
  ...
  const getSelectedDaijaName = () => {
    if (useCustomSpeaker) {
      return formData.speaker || '➕ Upiši ime daije';
    }
  ...
  const getSelectedOrganizationName = () => {
    if (useCustomOrganization) return formData.organization || '➕ Upiši ime udruženja';
                ...
                <View style={{ padding: 20 }}>
                  ...
                  <Menu
                    ...
                    anchor={
                      <TouchableOpacity
                        onPress={() => {
                          console.log('📱 Opening daije menu');
                          setDaijaMenuVisible(true);
                        }}
                        style={{
                          borderWidth: 1,
                          borderColor: useCustomSpeaker ? colors.primary.main : '#ccc',
                          borderRadius: 4,
                          padding: 16,
                          marginBottom: 16,
                          flexDirection: 'row',
                      ...
                      >
                        <Text style={{ 
                          color: useCustomSpeaker ? colors.primary.main : colors.text.primary,
                          flex: 1
                        }}>
                          {getSelectedDaijaName()}
                        </Text>
                        <Ionicons 
                          name="chevron-down" 
                          size={20} 
                          color={useCustomSpeaker ? colors.primary.main : colors.text.secondary} 
                        />
                      </TouchableOpacity>
                    }
                  >
                    ...
                    {/* Custom option at the top for better visibility */}
                    <Menu.Item
                      onPress={() => {
                        console.log('🎯 Custom daije option selected');
                    ...
                    >
                      {Array.isArray(filteredDaije) && filteredDaije.length > 0 ? (
                        filteredDaije.map((daija) => (
                          <Menu.Item
                            key={daija._id}
                            onPress={() => {
                        ...
                        <View style={{ padding: 16, alignItems: 'center' }}>
                          <Text style={{ color: colors.text.secondary }}>
                            {daijaSearch ? 'Nema rezultata pretrage' : 'Nema dostupnih daija'}
                          </Text>
                        </View>
                      )}
                    </ScrollView>
                  ...
                  {useCustomSpeaker && (
                    <View>
                      <Text style={{ 
                        ...
                        ✅ Upisivanje custom daije aktivno
                      </Text>
                      <TextInput
                        label="Ime i prezime daije *"
                        value={formData.speaker}
                        onChangeText={(text) => {
                          console.log('✏️ Custom speaker input:', text);
                          handleInputChange('speaker', text);
                        }}
                        mode="outlined"
                        style={{ marginBottom: 16 }}
                        placeholder="Unesite ime i prezime daije..."
                      />
                    </View>
                  )}
                  ...
                  <Menu
                    ...
                    anchor={
                      <TouchableOpacity
                        ...
                        style={{
                          borderWidth: 1,
                          borderColor: useCustomOrganization ? colors.primary.main : '#ccc',
                          borderRadius: 4,
                          padding: 16,
                          marginBottom: 16,
                          flexDirection: 'row',
                      ...
                      >
                        <Text style={{ 
                          color: useCustomOrganization ? colors.primary.main : colors.text.primary,
                          flex: 1
                        }}>
                        ...
                        <Ionicons 
                          name="chevron-down" 
                          size={20} 
                          color={useCustomOrganization ? colors.primary.main : colors.text.secondary} 
                        />
                      </TouchableOpacity>
                    }
                  >
                    ...
                    >
                      <Menu.Item
                        ...
                        title="Nije navedeno"
                      />
                      {Array.isArray(filteredOrganizations) && filteredOrganizations.length > 0 ? (
                        ...
                        <View style={{ padding: 16, alignItems: 'center' }}>
                          <Text style={{ color: colors.text.secondary }}>
                            {organizationSearch ? 'Nema rezultata pretrage' : 'Nema dostupnih udruženja'}
                          </Text>
                        </View>
                      )}
                    </ScrollView>
