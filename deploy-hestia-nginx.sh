...
# DERS.BA HestiaCP Nginx Configuration Deployment
# Adds custom configuration to existing HestiaCP setup

set -e

echo "🚀 DERS.BA HestiaCP Nginx Configuration Deployment"
echo "=================================================="
...
SERVER_USER="root"
SERVER_PORT="22"
CUSTOM_CONFIG="nginx-hestia-custom.conf"
HESTIA_CONFIG_DIR="/home/wanne/conf/web/ders.ba"
CUSTOM_CONFIG_NAME="nginx.ssl.conf_custom"
...
if [ ! -f "$CUSTOM_CONFIG" ]; then
    echo_error "Custom configuration file '$CUSTOM_CONFIG' not found!"
    exit 1
fi

echo_info "Found custom configuration: $CUSTOM_CONFIG"
...
echo_info "Uploading custom configuration to HestiaCP..."
if scp -P $SERVER_PORT "$CUSTOM_CONFIG" "$SERVER_USER@$SERVER_HOST:$HESTIA_CONFIG_DIR/$CUSTOM_CONFIG_NAME"; then
    echo_success "Custom configuration uploaded to $HESTIA_CONFIG_DIR/$CUSTOM_CONFIG_NAME"
else
    echo_error "Failed to upload custom configuration"
    exit 1
fi
...
# Modify main location block to proxy to Next.js for non-API requests
echo_info "Modifying main location block..."
ssh -p $SERVER_PORT $SERVER_USER@$SERVER_HOST "
...
if [ $? -eq 0 ]; then
    echo_success "Main configuration updated"
else
    echo_error "Failed to update main configuration"
    exit 1
fi
...
if ssh -p $SERVER_PORT $SERVER_USER@$SERVER_HOST "nginx -t"; then
    ...
    ssh -p $SERVER_PORT $SERVER_USER@$SERVER_HOST "
        cp $HESTIA_CONFIG_DIR/nginx.ssl.conf.backup.* $HESTIA_CONFIG_DIR/nginx.ssl.conf 2>/dev/null || true
    "
    exit 1
fi
...
echo "========================================"
echo ""
echo "✅ Custom configuration added to HestiaCP"
echo "✅ Main location block modified for Next.js"
...
echo "   • Main: $HESTIA_CONFIG_DIR/nginx.ssl.conf"
echo "   • Custom: $HESTIA_CONFIG_DIR/$CUSTOM_CONFIG_NAME"
echo "   • Backup: $HESTIA_CONFIG_DIR/nginx.ssl.conf.backup.*"
echo "" 