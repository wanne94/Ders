...
const connectDB = async () => {
  try {
    ...
    if (err.message && err.message.toLowerCase().includes('buffermaxentries')) {
      if (MONGODB_URI.toLowerCase().includes('buffermaxentries')) {
        const cleanedURI = MONGODB_URI.replace(/[&?]bufferMaxEntries=[^&]*/gi, '');
      }
    }
  }
};
